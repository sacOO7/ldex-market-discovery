...
describe("Ldex Markets",() => {
    ...
    it("Should return dex market with latest transactions for leasehold", async () => {
        const leaseholdClientOptions = ClientOptionsBuilder("54.82.244.206","8010").setRecentFirst(true).build();
        let marketPairs = await marketDiscovery.getDexMarketPair(leaseholdClientOptions);
        const pair = (await marketPairs.next()).value;
        expect(pair.market).toBe("lsk");
        expect(pair.dexWallet).toBe("7485409328757727573L");
    ...
    it("Should return dex market with latest transactions for leasehold using workers", async () => {
        const leaseholdClientOptions = ClientOptionsBuilder("54.82.244.206","8010").setRecentFirst(true).build();
        let marketPairs = await marketDiscovery.getDexMarketPair(leaseholdClientOptions, true);
        const pair = (await marketPairs.next()).value;
        expect(pair.market).toBe("lsk");
